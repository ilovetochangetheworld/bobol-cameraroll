...
`$ npm install bobol-cameraroll --save`

### Mostly automatic installation

`$ react-native link bobol-cameraroll && cd ios && pod install`
...
2. Go to `node_modules` ➜ `bobol-cameraroll` and add `RNCCameraroll.xcodeproj`
...
2. Append the following lines to `android/settings.gradle`:
  	...
  	project(':@react-native-community_cameraroll').projectDir = new File(rootProject.projectDir, 	'../node_modules/bobol-cameraroll/android')
  	```
...
```javascript
import CameraRoll from "bobol-cameraroll";
```
...
[license-badge]:https://img.shields.io/npm/l/bobol-cameraroll.svg?style=flat-square
